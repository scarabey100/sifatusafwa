                        ...
                        <div class="delivery-options">
                            {foreach from=$delivery_options item=carrier key=carrier_id}
                                <div class="row delivery-option js-delivery-option">
                                    <div class="delivery-option__checkbox">
                                        <span class="custom-radio">
                                            <input type="radio" name="delivery_option[{$id_address}]" id="delivery_option_{$carrier.id}" value="{$carrier_id}"{if $delivery_option == $carrier_id} checked{/if}>
                                            <span></span>
                                        </span>
                                    </div>
                                    <label for="delivery_option_{$carrier.id}" class="delivery-option__details delivery-option-2">
                                        <div class="carrier{if $carrier.logo} carrier-hasLogo{/if}">
                                            <div class="carrier-logoName">
                                                {if $carrier.logo}
                                                    <div class="carrier-logo">
                                                        <img height="44" width="44" src="{$carrier.logo}" alt="{$carrier.name}" loading="lazy" />
                                                    </div>
                                                {/if}
                                                <div class="h6 carrier-name">{$carrier.name}</div>
                                            </div>
                                            <div class="carrier-delay">{$carrier.delay}</div>
                                            <div class="carrier-price">{$carrier.price}</div>
                                        </div>
                                    </label>
                                </div>
