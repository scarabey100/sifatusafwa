...
$(document).ready(function () {
    ...
    function addNewRow(id, produit,check = false) {
        // Get existing IDs as an array from #productIds_selections input
        var existingIds = $('#productIds_selections').val().split(',').map(id => id.trim()); // Trim whitespace from IDs

        // Check if the ID already exists in the #productIds_selections input
        if (!existingIds.includes(id.toString()) || check) {
            if (!check) {
                ...
                $('#productIds_selections').val(existingIds.join(',').replace(/^,+/, '')); // Remove leading commas
            }
                ...
                <tr id="${id}" class="odd">
                    ...
                    <td class="pointer">
                        <div class="btn-group-action">
                            <div class="btn-group">
                                <a href="#" data-egid="${id}" class="btn tooltip-link product-edit link-icon-delete delete-selections" data-toggle="pstooltip" title="" data-placement="right" data-original-title="Supprimer définitivement cette item.">
    ...
    var chosenProducts = $('#productIds_selections').val();
    ...
    $(document).on('click', '#add_product_selections', function (e) {
    ...
    $(document).on('click', '.delete-selections', function (e) {
        ...
        // Get the current value of #productIds_selections
        var existingIds = $('#productIds_selections').val().split(','); // Split the input value into an array
        ...
        // Update the #productIds_selections input with the new list of IDs
        $('#productIds_selections').val(updatedIds.join(',')); // Join the array back into a comma-separated string
    });
