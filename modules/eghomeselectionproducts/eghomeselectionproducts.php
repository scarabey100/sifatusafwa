...
class EgHomeSelectionProducts extends Module implements WidgetInterface
{
    private $templateFile;
    public function __construct()
    {  
        $this->name = 'eghomeselectionproducts';
        ...
        $this->displayName = $this->l('HP selection products');
        $this->description = $this->l('Module for home page selection products.');
        $this->templateFile = 'module:eghomeselectionproducts/views/templates/hook/eghomeselectionproducts.tpl';
    }
     ...
     */
     ...
     public function getTabs()
     {
         return [
             [
                 'className' => 'AdminEgHomeSelectionProducts',
                 'parent' => 'CONFIGURE',
                 'name' => $this->l('HP selection products'),
                 'module' => $this->name,
                 'active' => true,
                 'icon' => 'extension',
             ],
    ...
    private function getProductIds($hookName, array $configuration)
    {
        ...
        if ('displayHome' === $hookName) {
            ...
            $productIds_selections = [];
            $targetCount = (int)Configuration::get('EGHOMESELECTIONPRODUCTS_NB_PRODUCTS');
            $defaultProductIds = explode(',', Configuration::get('EGHOMESELECTIONPRODUCTS_PRODUCT_IDS'));
            
            // If wishlist count is less than target, complete with default IDs
            $productIds_selections = array_slice($defaultProductIds, 0, $targetCount);
            
            if (!empty($productIds_selections)) {
                // Remove products with zero stock
                $ids = implode(',', array_map('intval', $productIds_selections));
                $shopId = (int)Context::getContext()->shop->id;
                $sql = "SELECT id_product FROM " . _DB_PREFIX_ . "stock_available
                        WHERE id_product IN ($ids) AND quantity > 0 AND id_shop = $shopId";
                $inStock = Db::getInstance()->executeS($sql);
                $inStockIds = array_map(function($row) { return (int)$row['id_product']; }, $inStock);
                $productIds_selections = array_values(array_intersect($productIds_selections, $inStockIds));
            }
          
            // Collect category IDs from cart products to retrieve related products
            $categoryIds = explode(',', Configuration::get('EGHOMESELECTIONPRODUCTS_CATEGORY')); // Fetch category IDs from configuration
            
            // Retrieve additional products from the same categories as cart products
            $relatedProductIds = [];
            
            $targetCount = $targetCount-count($productIds_selections);

            $relatedProductIds = $this->getProductsByCategory($categoryIds,$productIds_selections,$targetCount );

            // Combine wishlist, default, cart, and related product IDs, ensuring uniqueness
            $productIds_selections = array_unique(array_merge($productIds_selections, $relatedProductIds));
            $productIds_selections = array_map(function ($id) {
                return (int)$id;
            }, $productIds_selections);
            
            return $productIds_selections;
        }
    
        
    }
    private function getProductsByCategory(array $id_categories, array $productIds_selections, int $requiredCount)
    {
        ...
        $excludedProductIds = implode(',', array_map('intval', $productIds_selections));
    ...
    {
        $productIds_selections = $this->getProductIds($hookName, $configuration);
       
        if (!empty($productIds_selections)) {
            $products = $this->ToCartProducts($productIds_selections);
    ...
    {  
        ...
        if (!$this->isCached($this->templateFile)) {
            ...
            $title = Configuration::get('EGHOMESELECTIONPRODUCTS_TITLE_'.$default_lang);
            $selectedCategory = Configuration::get('EGHOMESELECTIONPRODUCTS_CATEGORY');
    ...
    protected function ToCartProducts(array $productIds_selections = [])
    {
        if (!empty($productIds_selections)) {
            ...
            if (is_array($productIds_selections)) {
                foreach ($productIds_selections as $productId) {
                    if ($productId > 0) {
    ...
    private function getSelectedProducts()
    {
        ...
        $productIds_selections = Configuration::get('EGHOMESELECTIONPRODUCTS_PRODUCT_IDS');
        return explode(',', $productIds_selections); // Return as an array
    }

    public function getContent()
    {
        $html = '';
        if (Tools::isSubmit('submit_eghomeselectionproducts')) {
            ...
            foreach ($title as $id_lang => $value) { 
                Configuration::updateValue('EGHOMESELECTIONPRODUCTS_TITLE_'.$id_lang, $value, true); // Save multilingual title
            }
            $productIds_selections = Tools::getValue('productIds_selections');
            Configuration::updateValue('EGHOMESELECTIONPRODUCTS_PRODUCT_IDS', $productIds_selections); // Store as a comma-separated string
            Configuration::updateValue('EGHOMESELECTIONPRODUCTS_NB_PRODUCTS', Tools::getValue('nb_produit'));
            Configuration::updateValue('EGHOMESELECTIONPRODUCTS_ACTIVE', Tools::getValue('active'));
            Configuration::updateValue('EGHOMESELECTIONPRODUCTS_RANDOM', Tools::getValue('random')); // Update random configuration
            // Fill EGHOMESELECTIONPRODUCTS_CATEGORY from productIds_choose
            $selectedCategory = Tools::getValue('id_category');
            Configuration::updateValue('EGHOMESELECTIONPRODUCTS_CATEGORY', $selectedCategory);
        }
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="row">
                                <div class="col-md-12 button_info">
                                    <button type="button" class="btn btn-outline-primary sensitive add" id="add_product_selections" data-id="" data-title="">
    ...
    public function renderForm()
    {
        ...
        $productIds_selections = Configuration::get('EGHOMESELECTIONPRODUCTS_PRODUCT_IDS'); // Single product ID
        $nb_produit = Configuration::get('EGHOMESELECTIONPRODUCTS_NB_PRODUCTS') ?: 8; 
        $selectedCategory = Configuration::get('EGHOMESELECTIONPRODUCTS_CATEGORY'); // Fetch selected category
        $active = Configuration::get('EGHOMESELECTIONPRODUCTS_ACTIVE');
        $random = Configuration::get('EGHOMESELECTIONPRODUCTS_RANDOM'); // Retrieve random configuration
       // dump($nb_produit); die;
            ...
            'form' => [
                ...
                'input' => [
                    ...
                    [
                        ...
                        'name' => 'productIds_selections', // Keep the original name for submission
                        'value' => $productIds_selections // Set value from configuration
                    ],
                    [
                        'type' => 'html',
                    ...
                    [
                        'type' => 'switch',
                        'label' => $this->trans('Affichage', array(), 'Modules.EgHomeSelectionProducts.Admin'),
                        'name' => 'active',
                        'is_bool' => true,
                        'values' => [
                            [
                                'id' => 'active_on',
                                'value' => 1,
                                'label' => $this->trans('Activé', array(), 'Modules.EgHomeSelectionProducts.Admin')
                            ],
                            [
                                'id' => 'active_off',
                                'value' => 0,
                                'label' => $this->trans('Désactivé', array(), 'Modules.EgHomeSelectionProducts.Admin')
                            ]
                        ]
                    ],
                ],
        ...
        $helper->module = $this;
        $helper->name_controller = 'eghomeselectionproducts';
        $helper->title = $this->displayName;
        $helper->submit_action = 'submit_eghomeselectionproducts';
        ...
        foreach (Language::getLanguages(false) as $lang) {
            $title = Configuration::get('EGHOMESELECTIONPRODUCTS_TITLE_'.$lang['id_lang']);
            $helper->fields_value['title'][$lang['id_lang']] = $title ?? '';
        } 
        $helper->fields_value['productIds_selections'] = $productIds_selections; // Set chosen product ID
