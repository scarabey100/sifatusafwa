...
function validateImportation(mandatory)
{
	...
	if (!('custom_value' in seted_value)) {
		mandatory.push('id_value');
	}
...
function importNow(offset, limit, total, validateOnly, crossStepsVariables, moreStep) {
    ...
    $.ajax({
       type: 'POST',
       url: importControllerUrl + '&ajax=1&action=import&offset='+offset+'&limit='+limit+(validateOnly?'&validateOnly=1':'')+((moreStep>0)?'&moreStep='+moreStep:''),
       cache: false,
       dataType: "json",
       data: data,
