...
$(document).ready(function() {
  ...
  // display preload zone if widget preload enabled
  $('#switch-preload').on('click', function() {
    var backgroundColor = $('#switch-preload').css('background-color');
    if (backgroundColor != 'rgb(120, 163, 0)') {
        $('#preload_widget_controllers').show(100);
    } else {
        $('#preload_widget_controllers').hide(100);
    }
  });
  ...
  $(document).on('click', '#show_more_link', function(e) {
    e.preventDefault();
    var isDisplayed = $('.show_more_links')[0].style.display;
    if (isDisplayed === 'none') {
      $('.show_more_links').css('display', 'list-item');
    } else {
      $('.show_more_links').css('display', 'none');
    }
  });
