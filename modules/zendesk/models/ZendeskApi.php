...
class ZendeskApi extends ZendeskCoreApi
{
    ...
    public function isValid($noApiCall = false)
    {
        if (!isset(self::$valid)) {
            // Check fields
            $conf = [
                Zendesk::USERNAME => Configuration::get(Zendesk::USERNAME),
                Zendesk::APIKEY => Configuration::get(Zendesk::APIKEY),
                Zendesk::SUBDOMAIN => Configuration::get(Zendesk::SUBDOMAIN),
            ];
            if ($conf[Zendesk::SUBDOMAIN] == '' && $conf[Zendesk::USERNAME] == '' && $conf[Zendesk::APIKEY] == '') {
                self::$valid = false;
    ...
    public function getUser($user_id)
    {
        ...
        if (!ctype_digit($user_id)) {
            return (object) [
                'error' => sprintf(
                    $this->l('Invalid user_id : %s', 'ZendeskApi'),
                    $user_id
                ),
            ];
        }
    ...
    public function appInformation($app_id)
    {
        ...
        if (!ctype_digit($app_id)) {
            return (object) [
                'error' => sprintf(
                    $this->l('Invalid app_id : %s', 'ZendeskApi'),
                    $app_id
                ),
            ];
        }
    ...
    {
        $app_id = (string) $app_id;
        $order_id_field_id = (string) $order_id_field_id;
        if (!ctype_digit($app_id)) {
            return (object) [
                'error' => sprintf(
                    $this->l('Invalid app_id : %s', 'ZendeskApi'),
            ...
            return (object) [
                'error' => sprintf(
                    $this->l('Invalid order_id_field_id : %s', 'ZendeskApi'),
                    $order_id_field_id
                ),
            ];
        }
    ...
    public function updateApp($app_id, $settings)
    {
        ...
        if (!ctype_digit($app_id)) {
            ...
            return (object) [
                'error' => $this->l('Parameter "$settings" should be an array', 'ZendeskApi'),
            ];
        }
    ...
    public function createTicketField($ticket_field)
    {
        if (!is_array($ticket_field)) {
            return (object) [
                'error' => $this->l('Parameter "$ticket_field" should be an array', 'ZendeskApi'),
            ];
        }
