...
$(document).ready(function () {
    ...
    function addNewRow(id, produit,check = false) {
        // Get existing IDs as an array from #productIds_hadith input
        var existingIds = $('#productIds_hadith').val().split(',').map(id => id.trim()); // Trim whitespace from IDs

        // Check if the ID already exists in the #productIds_hadith input
        if (!existingIds.includes(id.toString()) || check) {
            if (!check) {
                ...
                $('#productIds_hadith').val(existingIds.join(',').replace(/^,+/, '')); // Remove leading commas
            }
                ...
                <tr id="${id}" class="odd">
                    ...
                    <td class="pointer">
                        <div class="btn-group-action">
                            <div class="btn-group">
                                <a href="#" data-egid="${id}" class="btn tooltip-link product-edit link-icon-delete delete-hadith" data-toggle="pstooltip" title="" data-placement="right" data-original-title="Supprimer définitivement cette item.">
    ...
    var chosenProducts = $('#productIds_hadith').val();
    ...
    $(document).on('click', '#add_product_hadith', function (e) {
    ...
    $(document).on('click', '.delete-hadith', function (e) {
        ...
        // Get the current value of #productIds_hadith
        var existingIds = $('#productIds_hadith').val().split(','); // Split the input value into an array
        ...
        // Update the #productIds_hadith input with the new list of IDs
        $('#productIds_hadith').val(updatedIds.join(',')); // Join the array back into a comma-separated string
    });
