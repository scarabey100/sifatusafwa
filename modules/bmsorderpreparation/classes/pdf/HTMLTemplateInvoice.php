...
class HTMLTemplateBmsInvoice extends HTMLTemplateInvoice
{
    /**
    ...
    public function getContent()
    {
        ...
        $data = array(
            ...
            'barcode_image_path' => $this->getBarcodeImagePath(),
        );
     ...
     * @return string
     */
    protected function getBarcodeImagePath()
    {
        $bc = new pi_barcode();
        $path = _PS_MODULE_DIR_ . 'bmsorderpreparation/views/templates/admin/pdf/Invoice/barcode/barcode_' . (int)$this->order->id . '.jpg';

        $bc->setSize(50, 150, 10);
        $bc->setFiletype('JPG');
        $bc->setType('C128');
        $bc->setCode(sprintf("%'09d", (int)$this->order->id));
        $bc->writeBarcodeFile($path);
    ...
    {
        $path = _PS_MODULE_DIR_ . 'bmsorderpreparation/views/templates/admin/pdf/Invoice/' . Configuration::get('PS_INVOICE_MODEL');

        if (!file_exists($template = $path . '.' . $iso_country . '.tpl')) {
            $template = $path . '.tpl';
        }
