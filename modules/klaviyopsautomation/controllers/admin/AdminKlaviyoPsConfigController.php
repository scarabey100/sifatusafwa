...
class AdminKlaviyoPsConfigController extends ModuleAdminController
{
    /**
     * @see config.tpl
     * We need to handle flash message here
     * Instead of using PrestaShop's built in flash system
     * Because since PrestaShop 9, HTML is not longer escaped
     */
    protected $klaviyoFlashes = [
        'danger' => [],
        'warning' => [],
        'info' => [],
        'success' => [],
    ...
    const PRIVATE_KEY_VALIDATION_REGEX = '/^pk_([0-9a-f]{34}|[A-Za-z0-9]+_[0-9a-f]{34})$/';
    ...
    public function initContent()
    {
        ...
        if (!$this->module->getConfigurationValueOrNull('KLAVIYO_PUBLIC_API')) {
            ...
            $this->klaviyoFlashes['info'][] = sprintf(
        ...
        if (!$this->module->getConfigurationValueOrNull('KLAVIYO_ORDER_STATUS_MAP')) {
            $this->klaviyoFlashes['warning'][] = $this->l('Please complete the order status mapping form or accept the default values.');
        }

        // Display warning if php running as CGI to help avoid auth errors
        if (KlaviyoUtils::shouldWarnCGI()) {
            $this->klaviyoFlashes['danger'][] = sprintf(
    ...
    public function renderContent()
    {
        ...
        $this->context->smarty->assign([
            'klaviyoFlashes' => $this->klaviyoFlashes,
    ...
    public function renderPsAccounts()
    {
        ...
        try {
            ...
            $this->klaviyoFlashes['danger'][] = $errorMessage;
        }
    ...
    public function renderForm()
    {
        ...
        $fieldsForm[0]['form'] = [
            ...
            'input' => [
                ...
                [
                    ...
                    'placeholder' => 'pk_AbC123_1112223334445556667778889990aabbcc',
                    'id' => 'kl_private_key',
        ...
        if (
            ...
            try {
                ...
                $this->klaviyoFlashes['danger'][] = $this->l(
    ...
    private function validateAndGenerateCoupons()
    {
        try {
            ...
            $this->klaviyoFlashes['danger'][] = $e->getMessage();
        }
    }
    ...
    private function validateAndSaveConfig($validationMethod, $saveMethod, $values)
    {
        ...
        if ($config_form_errors) {
            $this->klaviyoFlashes['danger'][] = $this->l(implode(' ', $config_form_errors));
        } else {
            call_user_func(array($this, $saveMethod), $values);
            $this->klaviyoFlashes['success'][] = $this->l('Settings updated');
        }
    }
    ...
    protected function l(
        ...
        if (version_compare(_PS_VERSION_, '9.0.0', '>=')) {
            ...
            // In the case where it is a native translation
            // We do not pass $specific in order to translate this type of translation
            // Therefore, if $translated is different from $string
            // Then it is a native translation
            $translated = $this->module->l($string);
