...
class KlaviyoPsModule extends Module
{
    ...
    public function hookActionEmailSendBefore($params)
    {
        try {
            ...
            // Block transactional emails if feature is enabled
            if (!$isNotBlockingTransactionalEmails && in_array($template, self::STATIC_TRANSACTIONAL_EMAIL_TEMPLATES, true)) {
                return false;
            }

            // Block back in stock emails if feature is enabled
            if (!$isBisDisabled && in_array($template, self::BACK_IN_STOCK_EMAIL_TEMPLATES, true)) {
                return false;
            }
    ...
    {
        $hooksHandler = new HooksHandler($this);
        $hooksHandler->handleActionSubmitCustomerAddressForm($params);
    }
    ...
    {
        $hooksHandler = new HooksHandler($this);
        $hooksHandler->handleDisplayOrderConfirmation($params);
    }
