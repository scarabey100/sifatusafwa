...
class HooksHandler
{
        ...
        try {
            ...
            if (
                ...
                try {
                    ...
                    $logger->error(self::ERROR_SUBSCRIBING_CUSTOMER . $customer->email);
                }
            }
            ...
            try {
                ...
                $logger->error(self::ERROR_UPDATING_CUSTOM_PROPERTIES . $customer->email);
            }
        } catch (Exception $e) {
            /** @var LoggerService $logger */
            $logger = $this->klaviyoModule->getService('klaviyops.prestashop_services.logger');
            ...
            if (Validate::isLoadedObject($customer)) {
                $logger->error("An error occured in handleActionCustomerAccount for customer {$customer->email}");
            } else {
                $logger->error('An error occured in handleActionCustomerAccount');
            }
        }
    }
    ...
    public function handleActionNewsletterSubscription(array $params)
    {
        if (
            ...
            try {
                ...
                $logger->error(self::ERROR_SUBSCRIBING_CUSTOMER . $params['email']);
            }
        }
    }
    ...
    public function handleActionSubmitCustomerAddressForm(array $params)
    {
        ...
        $klSmsConsent = (int) Tools::getValue('kl_sms_consent', 0);
        ...
        if ($trigger === 'place_order') {
            ...
            $context->cookie->kl_sms_consent = Tools::hash($addressToHash);

            return;
        }

        $this->sendSMSConsentRequestAPI($address);
    }
    ...
    public function handleDisplayOrderConfirmation($params)
    {
        ...
        if (
            ...
            $this->sendSMSConsentRequestAPI($consentAddress);
        }
    }
     ...
     * @return void
     */
    private function sendSMSConsentRequestAPI($address)
    {
        $mobile = '';

        try {
            $mobile = $this->getPhoneMobile($address);
            ...
            $api = new KlaviyoApiWrapper();
            $api->subscribeSMSCustomer($attributes);
        } catch (KlaviyoApiException $e) {
            /** @var LoggerService $logger */
            $logger = $this->klaviyoModule->getService('klaviyops.prestashop_services.logger');

            $logger->error(
                self::ERROR_SUBSCRIBING_SMS_CUSTOMER . $mobile
            );
        }
    }
     ...
     * @return string|null
     */
    private function getPhoneMobile($address)
    {
        ...
        return KlaviyoUtils::formatPhone(
            $mobile,
            $isoCode
        );
    }
