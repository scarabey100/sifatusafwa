...
class OrderEventService
{
    ...
    public function buildPayload(ArrayObject $order)
    {
        ...
        return [
            ...
            'BillingAddress' => $this->buildAddress($orderObject->id_address_invoice, (int) $order['id_lang']),
            'ShippingAddress' => $this->buildAddress($orderObject->id_address_delivery, (int) $order['id_lang']),
    ...
    /**
     * Build shipping data including carrier, tracking number, and tracking URL
     *
     * @param ArrayObject $order
     * @return array|null
     */
    protected function buildShippingData(ArrayObject $order)
    {
        ...
        // Build tracking URL from carrier URL template
        $carrierUrl = $shippingData['url'] ?? null;

        if (!empty($trackingNumber) && !empty($carrierUrl)) {
            // PrestaShop carrier URLs use @ as placeholder for tracking number
            $result['TrackingURL'] = str_replace('@', $trackingNumber, $carrierUrl);
        }
    ...
    /**
     * Build address data from address ID
     *
     * @param int $addressId
     * @return array|null
     */
    protected function buildAddress($addressId, int $idLang)
    {
        try {
            ...
            // Build address array with required fields
            $result = [
            ...
            if (!empty($address->id_country)) {
                $country = new Country($address->id_country, $idLang);
                if (Validate::isLoadedObject($country)) {
                    $result['country'] = $country->name ?? null;
                    $result['country_code'] = $country->iso_code ?? null;
                }
            }
            ...
            if (!empty($address->id_state)) {
                ...
                if (Validate::isLoadedObject($state)) {
                    $result['region'] = $state->name ?? null;
                }
            }

            // Add phone number with E.164 formatting
            $phone = $address->phone_mobile ?: $address->phone ?: null;
            if (!empty($phone)) {
                $isoCode = $result['country_code'] ?? '';
                $result['phone'] = KlaviyoUtils::formatPhone($phone, $isoCode);
            }
