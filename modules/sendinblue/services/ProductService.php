...
class ProductService
{
    ...
    const FILTERING_EQUAL_NUMERIC = '= %d';
    ...
    public function getProducts($term, $page, $itemsPerPage)
    {
        ...
        $sqlTable = [
            ...
            'pl' => [
                ...
                'join' => 'LEFT JOIN',
                'on' => sprintf(
                    'pl.`id_product` = p.`id_product` AND pl.`id_lang` = %d AND pl.`id_shop` = %d',
                    (int) $this->idLang,
                    (int) $this->idShop
                ),
            ],
            'sa' => [
                ...
                'join' => 'JOIN',
                'on' => sprintf(
                    'p.`id_product` = sa.`id_product` AND sa.id_shop = %d',
                    (int) $this->idShop
                ),
            ],
            'cl' => [
                ...
                'join' => 'LEFT JOIN',
                'on' => sprintf(
                    'sa.`id_category_default` = cl.`id_category` AND cl.`id_lang` = %d AND cl.id_shop = %d',
                    (int) $this->idLang,
                    (int) $this->idShop
                ),
            ],
            ...
            'shop' => [
                'table' => 'shop',
                'join' => 'LEFT JOIN',
                'on' => sprintf('shop.id_shop = %d', (int) $this->idShop),
            ],
            'image_shop' => [
                'table' => 'image_shop',
                'join' => 'LEFT JOIN',
                'on' => sprintf(
                    'image_shop.`id_product` = p.`id_product` AND image_shop.`cover` = 1 AND image_shop.id_shop = %d',
                    (int) $this->idShop
                ),
            ],
            'i' => [
    ...
    public function getProductsForFullSync($offset, $itemsPerPage)
    {
        ...
        try {
            ...
            foreach ($all_products as $key => $product) {
                $products[$key]['id'] = isset($product['id_product']) ? (int) $product['id_product'] : 0;
                $products[$key]['name'] = isset($product['name']) ? $product['name'] : '';
                $basePrice = (float) $product['price'];
                $taxRate = (float) $product['rate'];
                $products[$key]['original_price'] = round($basePrice * (1 + $taxRate / 100), 2);
                $products[$key]['price'] = isset($product['id_product']) ? (string) \ProductCore::getPriceStatic($product['id_product']) : '';
                $products[$key]['permalink'] = isset($product['id_product']) ? $this->getProductUrl($product) : '';
                $products[$key]['date_created'] = isset($product['date_add']) ? gmdate('Y-m-d\TH:i:s', strtotime($product['date_add'])) : '';
                $products[$key]['date_modified'] = isset($product['date_upd']) ? gmdate('Y-m-d\TH:i:s', strtotime($product['date_upd'])) : '';
                ...
                $products[$key]['categories'][] = isset($product['id_category_default']) ? $product['id_category_default'] : 0;
                    
                $category = isset($product['id_category_default']) ?  new \Category($product['id_category_default'], $id_lang) : "";
                if (!empty($category)) {
                    $products[$key]['categories'][0] = [
                    'id' => isset($product['id_category_default']) ? (int) $product['id_category_default'] : 0,
                    'name' => isset($category->name) ? $category->name : '',
                    'description' => isset($category->description) ? $category->description : '',
                    'parent' => isset($category->id_parent) ? (int) $category->id_parent : 0,
                    'slug' => isset($category->link_rewrite) ? $category->link_rewrite : '',
                ];
                }
                ...
                $products[$key]['quantity'] = isset($product['id_product']) ? (string) \ProductCore::getQuantity($product['id_product']) : "";
                $products[$key]['sku'] = isset($product['reference']) ? $product['reference'] : "";
            }
    ...
    private function getProductImageUrl($productId, $productLink)
    {
        $imageLinks = [];
        try {
            $imagesQuery = sprintf('SELECT id_image FROM %simage_shop WHERE id_product = %d', _DB_PREFIX_, (int) $productId);
    ...
    public function getTotalCount($term)
    {
        $result = \DbCore::getInstance()->executeS(
            sprintf(
                'SELECT count(*)  AS `count` FROM  `%sproduct` p LEFT JOIN `%sproduct_lang` pl ON (pl.`id_product` = p.`id_product` AND pl.`id_lang` = %d AND pl.`id_shop` = %d ) JOIN `%sproduct_shop` sa ON (p.`id_product` = sa.`id_product` AND sa.id_shop = 1) WHERE (1 AND pl.`name` LIKE "%%%s%%" AND sa.`active` = 1 AND state = 1)',
                _DB_PREFIX_,
                _DB_PREFIX_,
                (int) $this->idLang,
                (int) $this->idShop,
    ...
    public function getTotalProductCount()
    {
        $result = \DbCore::getInstance()->executeS(
            sprintf(
                ...
                ON pl.id_product = p.id_product AND pl.id_lang = %d AND pl.id_shop = %d
                WHERE ps.active = 1
                AND p.state = 1
                AND ps.visibility IN ("both", "catalog")',
                _DB_PREFIX_, // ps_product
                _DB_PREFIX_, // ps_product_shop
                (int) $this->idShop,
                _DB_PREFIX_, // ps_product_lang
                (int) $this->idLang,
                (int) $this->idShop
            )
        );
