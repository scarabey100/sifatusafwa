...
function getGoogleAdsBestGroupsPerCtr($vars,$humanResult=true)
{   
    return OpartStatTools::getBestMetricResult('googleAdsBestGroupsPerCtr', $vars);
}

function getGoogleAdsBestGroupsPerCtrValues($dateFrom,$dateTo,$filtersArray,$start,$limit,$vars) {
    ...
    $response = OpartStatTools::getMetricsResultFromSaas("metrics/googleAds/bestGroupsPerCtr.php",$useCache,$datas);
    ...
    foreach ($mergedCampaigns as $groupId => $campaignData) {
        if($campaignData['impressions']>0)
            $mergedCampaigns[$groupId]['total'] = ($campaignData['clicks']/$campaignData['impressions'])*100;
        else
