...
function getGoogleAdsRoas($vars, $humanResult = true)
{
    return OpartStatTools::getMetricResult('GoogleAdsRoas', $vars);
}

function getGoogleAdsRoasValues($dateFrom, $dateTo, $filtersArray, $vars)
{
    $result['value'] = 0;
    $result['conf'] = [
        'total' => 'price'
    ...
    /* $response = OpartStatTools::getMetricsResultFromSaas("metrics/googleAds/roas.php",$useCache,$datas);
    ...
    if (is_array($resultDatas) && count($resultDatas) > 0)
        foreach ($resultDatas as $resultData)
            $totalRevenues += $resultData['total'];
    ...
    $totalCosts = Db::getInstance()->getValue($sql);

    if ($totalCosts == null)
        $totalCosts = 0;

    $datas = $totalRevenues - $totalCosts;
