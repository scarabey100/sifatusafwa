...
function getGoogleAdsBestAdsPerCtr($vars,$humanResult=true)
{   
    return OpartStatTools::getBestMetricResult('googleAdsBestAdsPerCtr', $vars);
}

function getGoogleAdsBestAdsPerCtrValues($dateFrom,$dateTo,$filtersArray,$start,$limit,$vars) {
    ...
    $response = OpartStatTools::getMetricsResultFromSaas("metrics/googleAds/bestAdsPerCtr.php",$useCache,$datas);
    ...
    foreach ($mergedCampaigns as $adId => $campaignData) {
        if($campaignData['impressions']>0)
            $mergedCampaigns[$adId]['total'] = ($campaignData['clicks']/$campaignData['impressions'])*100;
        else
