...
function getBestRevenuesDayOfTheWeek($vars)
{
    return OpartStatTools::getBestMetricResult('bestRevenuesDayOfTheWeek',$vars);
}

function getBestRevenuesDayOfTheWeekValues($dateFrom, $dateTo, $filtersArray, $start, $limit,$vars) {
    ...
    if(isset($vars['otherVars']['useCache']))
        ...
        $sql = "SELECT SQL_NO_CACHE 
            DAYOFWEEK(orders.`".bqSQL($dateColumn)."`) AS dayId, 
            (".$fields.") as total
        FROM  
    ...
    $dayOfTheWeekTranslatedArray = OpartStatTools::getDayOfTheWeekTranslatedArray();
    $mergedDayList = [];
    foreach ($orderList as $o) {
        if($o['dayId'] == null)
            continue;
        $total = ($o['total'] == null)?0:$o['total'];
        if (isset($mergedDayList[$o['dayId']]) && $mergedDayList[$o['dayId']]['total'] > 0) {
            $mergedDayList[$o['dayId']]['total'] = $total+$mergedDayList[$o['dayId']]['total'];
        }
        else {
            $mergedDayList[$o['dayId']] = [
                'total' => $total,
                'name' => $dayOfTheWeekTranslatedArray[$o['dayId']],
                'id' => $o['dayId'],
            ];
        }
    }

    $result['value'] = $mergedDayList;
