...
function getGoogleAdsBestCampaignsPerCpc($vars,$humanResult=true)
{   
    return OpartStatTools::getBestMetricResult('googleAdsBestCampaignsPerCpc', $vars);
}

function getGoogleAdsBestCampaignsPerCpcValues($dateFrom,$dateTo,$filtersArray,$start,$limit,$vars) {
    ...
    $response = OpartStatTools::getMetricsResultFromSaas("metrics/googleAds/bestCampaignsPerCpc.php",$useCache,$datas);
    ...
    foreach ($mergedCampaigns as $campaignId => $campaignData) {
        if($campaignData['clicks']>0)
            $mergedCampaigns[$campaignId]['total'] = $campaignData['costs']/$campaignData['clicks'];
        else
