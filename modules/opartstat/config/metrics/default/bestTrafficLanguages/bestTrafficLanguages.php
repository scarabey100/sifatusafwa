...
function getBestTrafficLanguages($vars)
{
    return OpartStatTools::getBestMetricResult('bestTrafficLanguages', $vars);
}

function getBestTrafficLanguagesValues($dateFrom, $dateTo, $filtersArray, $start, $limit, $vars)
{
    $result['conf']['total'] = '';
    $result['value'] = [];
    ...
    /* if (Configuration::get('OPARTSTAT_USE_SAAS')) {
        ...
        $response = OpartStatTools::getMetricsResultFromSaas("metrics/prestashop/bestTrafficLanguages.php", $useCache, $datas);
        ...
        $sql = "SELECT SQL_NO_CACHE 
                ...
                opartstat_sessions.userLanguage
            FROM 
    ...
    foreach ($visits as $v) {
        $language = opartStatTools::convertIsoToHumanLanguage($v['userLanguage'], $idLang);
        $language = ($language == '') ? 'unknow' : $language;

        if (!empty($mergedLanguageList[$language]))
            $mergedLanguageList[$language] = $mergedLanguageList[$language] + 1;
        else
            $mergedLanguageList[$language] = 1;
    }

    $referrerList = [];
    foreach ($mergedLanguageList as $name => $visit) {
        $referrerList[$name] = [
            'id' => $name,
            'name' => $name,
            'total' => $visit,
