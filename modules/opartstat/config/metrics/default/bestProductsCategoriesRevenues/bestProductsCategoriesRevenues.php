...
function getBestProductsCategoriesRevenues($vars)
{
    return OpartStatTools::getBestMetricResult('bestProductsCategoriesRevenues',$vars);
}

function getBestProductsCategoriesRevenuesValues($dateFrom, $dateTo, $filtersArray, $start, $limit,$vars) {
    ...
    $sql = "SELECT SQL_NO_CACHE  
                ...
                (
                    (
                        order_detail.total_price_tax_excl - ".pSQL($refundedFields)." - (
                            (orders.total_discounts_tax_excl-IFNULL(order_cart_rule.value_tax_excl,0))*(order_detail.total_price_tax_excl/orders.total_products)
                        )
    ...
    foreach ($productList as $p) {
        ...
        if (isset($mergedProductList[$idCat.'_'.$idProd])) {
            (int)$mergedProductList[$idCat.'_'.$idProd]['total'] += $total;
        } else {
