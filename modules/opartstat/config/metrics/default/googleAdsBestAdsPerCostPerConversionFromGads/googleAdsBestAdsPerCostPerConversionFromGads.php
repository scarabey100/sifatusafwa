...
function getGoogleAdsBestAdsPerCostPerConversionFromGads($vars, $humanResult = true)
{
    return OpartStatTools::getBestMetricResult('googleAdsBestAdsPerCostPerConversionFromGads', $vars);
}

function getGoogleAdsBestAdsPerCostPerConversionFromGadsValues($dateFrom, $dateTo, $filtersArray, $start, $limit, $vars)
{
    ...
    $response = OpartStatTools::getMetricsResultFromSaas("metrics/googleAds/bestAdsPerCostPerConversionFromGads.php", $useCache, $datas);
    ...
    foreach ($ads as $p) {
        if (isset($mergedResults[$p['adId']])) {
            $mergedResults[$p['adId']]['conversions'] += $p['conversions'];
            $mergedResults[$p['adId']]['costs'] += $p['costs'];
        } else {
