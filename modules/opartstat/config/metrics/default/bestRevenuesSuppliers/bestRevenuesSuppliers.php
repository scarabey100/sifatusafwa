...
function getBestRevenuesSuppliers($vars)
{
    return OpartStatTools::getBestMetricResult('bestRevenuesSuppliers',$vars);
}

function getBestRevenuesSuppliersValues($dateFrom, $dateTo, $filtersArray, $start, $limit,$vars) {
    ...
    foreach ($productList as $p) {
        $supId = ($p['supplier_name'] == null) ? 0 : $p['id_supplier'];
        $total = ($p['total'] == null) ? 0 : $p['total'];
        if (isset($mergedSupplierList[$supId])) {
            $mergedSupplierList[$supId]['total'] += (float)$total;
        } else {
