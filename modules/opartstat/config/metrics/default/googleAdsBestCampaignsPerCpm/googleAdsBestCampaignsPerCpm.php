...
function getGoogleAdsBestCampaignsPerCpm($vars,$humanResult=true)
{   
    return OpartStatTools::getBestMetricResult('googleAdsBestCampaignsPerCpm', $vars);
}

function getGoogleAdsBestCampaignsPerCpmValues($dateFrom,$dateTo,$filtersArray,$start,$limit,$vars) {
    ...
    $response = OpartStatTools::getMetricsResultFromSaas("metrics/googleAds/bestCampaignsPerCpm.php",$useCache,$datas);
    ...
    foreach ($mergedCampaigns as $campaignId => $campaignData) {
        if($campaignData['impressions']>0)
            $mergedCampaigns[$campaignId]['total'] = $campaignData['costs']/$campaignData['impressions'];
            else
