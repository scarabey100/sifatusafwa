...
function getGoogleAdsBestGroupsPerCpc($vars,$humanResult=true)
{   
    return OpartStatTools::getBestMetricResult('googleAdsBestGroupsPerCpc', $vars);
}

function getGoogleAdsBestGroupsPerCpcValues($dateFrom,$dateTo,$filtersArray,$start,$limit,$vars) {
    ...
    $response = OpartStatTools::getMetricsResultFromSaas("metrics/googleAds/bestGroupsPerCpc.php",$useCache,$datas);
    ...
    foreach ($mergedCampaigns as $groupId => $campaignData) {
        if($campaignData['clicks']>0)
            $mergedCampaigns[$groupId]['total'] = $campaignData['costs']/$campaignData['clicks'];
        else
