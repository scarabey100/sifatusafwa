...
function getBestProfitsSuppliers($vars)
{
    return OpartStatTools::getBestMetricResult('bestProfitsSuppliers', $vars);
}

function getBestProfitsSuppliersValues($dateFrom, $dateTo, $filtersArray, $start, $limit,$vars) {
    ...
    $sqlJoins = OpartStatTools::getJoins($filtersArray,['product','order_detail'],[],true);
    ...
    foreach ($productList as $p) {
        ...
        if (isset($mergedSupplierList[$manId])) {
            $mergedSupplierList[$manId]['total'] += (float)$total;
        } else {
    ...
    $result['value'] = $mergedSupplierList;
