...
function getBestNumberOrderPaymentMethods($vars)
{
    return OpartStatTools::getBestMetricResult('bestNumberOrderPaymentMethods',$vars);
}

function getBestNumberOrderPaymentMethodsValues($dateFrom, $dateTo, $filtersArray, $start, $limit,$vars) {
    $result['conf']['total'] = '';
    $result['value'] = [];
    ...
    foreach ($orderList as $o) {
        if (isset($mergedCountryList[$o['payment']]) && $mergedCountryList[$o['payment']]['total'] > 0) {
            $mergedCountryList[$o['payment']]['total'] = $mergedCountryList[$o['payment']]['total']+1;
        }
        else {
            $mergedCountryList[$o['payment']] = [
                'id' => $o['payment'],
                'name' => $o['payment'],
                'total' => 1
            ];
