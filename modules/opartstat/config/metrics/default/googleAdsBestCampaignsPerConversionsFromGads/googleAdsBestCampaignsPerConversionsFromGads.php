...
function getGoogleAdsBestCampaignsPerConversionsFromGads($vars,$humanResult=true)
{   
    return OpartStatTools::getBestMetricResult('googleAdsBestCampaignsPerConversionsFromGads', $vars);
}

function getGoogleAdsBestCampaignsPerConversionsFromGadsValues($dateFrom,$dateTo,$filtersArray,$start,$limit,$vars) {
    ...
    /* $response = OpartStatTools::getMetricsResultFromSaas("metrics/googleAds/bestCampaignsPerConversionsFromGads.php",$useCache,$datas);
    ...
    foreach ($campaigns as $p) {
        if (isset($mergedCampaigns[$p['campaignId']])) {
            $mergedCampaigns[$p['campaignId']]['total'] += $p['total'];
        } else {
            $campaignName = ($p['name'] == null)?'Unknow':$p['name'];
            $mergedCampaigns[$p['campaignId']] = [
                'id' => $p['campaignId'],
