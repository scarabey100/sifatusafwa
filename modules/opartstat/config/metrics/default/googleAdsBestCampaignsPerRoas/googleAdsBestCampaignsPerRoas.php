...
function getGoogleAdsBestCampaignsPerRoas($vars, $humanResult = true)
{
    return OpartStatTools::getBestMetricResult('googleAdsBestCampaignsPerRoas', $vars, 'price', false);
}

function getGoogleAdsBestCampaignsPerRoasValues($dateFrom, $dateTo, $filtersArray, $start, $limit, $vars)
{
    ...
    $datas = [
        ...
        'totalType' => "orderTotal",
        'useCache' => $useCache
    ];
    /*
    $response = OpartStatTools::getMetricsResultFromSaas("metrics/googleAds/bestCampaignsPerRoas.php", $useCache, $datas);
    ...
    foreach($googleAdsElementsWithOrders as $id => $googleAdsElementWithOrders) {
        ...
        $resultDatas[$id]['total'] = $googleAdsElementWithOrders['total'] - $googleAdsElementsCostsByAdId[$id];
    }
    
    $campaigns = $resultDatas;

    if ($campaigns == null || count($campaigns) == 0)
        return $result;

    $result['value'] = $campaigns;
    $result['conf']['allDataLoaded'] = true;
