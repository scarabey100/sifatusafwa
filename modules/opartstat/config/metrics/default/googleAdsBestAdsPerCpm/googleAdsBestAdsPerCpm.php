...
function getGoogleAdsBestAdsPerCpm($vars,$humanResult=true)
{   
    return OpartStatTools::getBestMetricResult('googleAdsBestAdsPerCpm', $vars);
}

function getGoogleAdsBestAdsPerCpmValues($dateFrom,$dateTo,$filtersArray,$start,$limit,$vars) {
    ...
    $response = OpartStatTools::getMetricsResultFromSaas("metrics/googleAds/bestAdsPerCpm.php",$useCache,$datas);
    ...
    foreach ($mergedCampaigns as $adId => $campaignData) {
        if($campaignData['impressions']>0)
            $mergedCampaigns[$adId]['total'] = $campaignData['costs']/$campaignData['impressions'];
        else
