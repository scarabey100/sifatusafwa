...
function getBestTrafficCountries($vars)
{
    return OpartStatTools::getBestMetricResult('bestTrafficCountries', $vars);
}

function getBestTrafficCountriesValues($dateFrom, $dateTo, $filtersArray, $start, $limit, $vars)
{
    $result['conf']['total'] = '';
    $result['value'] = [];
    ...
    /* if (Configuration::get('OPARTSTAT_USE_SAAS')) {
        ...
        $response = OpartStatTools::getMetricsResultFromSaas("metrics/prestashop/bestTrafficCountries.php", $useCache, $datas);
    ...
    foreach ($visits as $v) {
        $Country = $v['country'];
        $Country = ($Country == '') ? 'unknow' : $Country;

        if (!empty($mergedCountryList[$Country]))
            $mergedCountryList[$Country] = $mergedCountryList[$Country] + 1;
        else
            $mergedCountryList[$Country] = 1;
    }

    $referrerList = [];
    foreach ($mergedCountryList as $name => $visit) {
        $referrerList[$name] = [
            'id' => $name,
            'name' => $name,
            'total' => $visit,
