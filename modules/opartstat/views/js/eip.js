...
$.fn.editable = function (options) {
  ...
  function toInput(editableElement) {
    ...
    const type = editableElement.hasAttribute("type")
    ...
    const wrapper = `
            ...
            <div class="form-group">
                <input type="${type}" class="form-control eip-editable input-sm" value="${editableElement.textContent.trim()}"/>
            </div>
        `;

    // Add the input wrapper to the parent instead of replacing its content
    editableElement.insertAdjacentHTML("afterend", wrapper);

    const input = parent.querySelector(".eip-editable");
    setInput(input);

    // Hide the editable element while editing
  ...
  function toEditableElement(parent, editableElement, parentClassList) {
    const inputWrapper = parent.querySelector(".form-group");
    if(inputWrapper == null)
      return false;
    
    parent.removeChild(inputWrapper);
